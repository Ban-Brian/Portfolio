...
# --- Sensitivity analysis: try alternative priors ---
print("Prior Sensitivity Analysis:")
print(f"{'Prior (alpha, beta)':<25} {'Prior Mean':<12} {'Post Mean':<12} {'95% CI':>25}")
print("-" * 75)
for a0, b0, label in [
    (2.0, 0.01, "Baseline"),
    (1.0, 0.001, "Vaguer"),
    ...
    b_p = b0 + T
    pm = a_p / b_p
    ci_l = gamma_dist.ppf(0.025, a=a_p, scale=1.0 / b_p)
    ci_h = gamma_dist.ppf(0.975, a=a_p, scale=1.0 / b_p)
    print(f"  ({a0:>5.1f}, {b0:>5.3f}) {label:<10}  {a0/b0:<12.1f} {pm:<12.4f} [{ci_l:.4f}, {ci_h:.4f}]")
print("\nAll three priors produce virtually identical posteriors, confirming")
print("that the posterior is dominated by the data, not the prior choice.\n")
...
test_cost_c = (C_FP * FP_c + C_FN * FN_c) / len(A_test)
...
print(f"Test exp. cost:          {test_cost_c:.4f}")

# ---- 7c: Constraint-based (<=2 alerts/week) ----
weeks_val = n_val / (24 * 7)
max_alerts_val = int(np.floor(2 * weeks_val))
print(f"\nConstraint-based: <= 2 alerts/week")
print(f"  Val set spans {weeks_val:.2f} weeks => max {max_alerts_val} alerts allowed")

# Find lowest tau such that total alerts on val <= max_alerts_val
best_tau_2pw = 1.0
feasible_2pw = False
for t in np.linspace(0.999, 0.001, 10000):
    n_alerts = (pt_cal_val >= t).sum()
    if n_alerts <= max_alerts_val:
        best_tau_2pw = t
        feasible_2pw = n_alerts > 0
        break

# Also find nearest feasible point (fewest alerts > 0)
nearest_tau_2pw = None
nearest_alerts_2pw = None
for t in np.linspace(0.999, 0.001, 10000):
    n_a = (pt_cal_val >= t).sum()
    if n_a > 0:
        if nearest_alerts_2pw is None or n_a < nearest_alerts_2pw:
            nearest_tau_2pw = t
            nearest_alerts_2pw = n_a

alerts_2pw = (pt_cal_test >= best_tau_2pw).astype(int)
TP_2 = ((alerts_2pw == 1) & (A_test == 1)).sum()
FP_2 = ((alerts_2pw == 1) & (A_test == 0)).sum()
FN_2 = ((alerts_2pw == 0) & (A_test == 1)).sum()
TN_2 = ((alerts_2pw == 0) & (A_test == 0)).sum()
weeks_test = n_test / (24 * 7)
alerts_per_week_test = alerts_2pw.sum() / weeks_test
test_cost_2pw = (C_FP * FP_2 + C_FN * FN_2) / len(A_test)

print(f"Chosen tau (val):        {best_tau_2pw:.4f}")
print(f"Test confusion: TP={TP_2}  FP={FP_2}  FN={FN_2}  TN={TN_2}")
print(f"Test alerts/week:        {alerts_per_week_test:.2f}")
print(f"Test precision:          {TP_2 / (TP_2 + FP_2 + 1e-9):.4f}")
print(f"Test recall:             {TP_2 / (TP_2 + FN_2 + 1e-9):.4f}")
print(f"Test exp. cost:          {test_cost_2pw:.4f}")
if not feasible_2pw:
    min_apw = nearest_alerts_2pw / weeks_val if nearest_alerts_2pw else 0
    print(f"  NOTE: Constraint infeasible. All predicted probs are identical,")
    print(f"  so the minimum nonzero alert volume is {nearest_alerts_2pw} ({min_apw:.1f}/wk).")
...
if "hr" not in df.columns or "workingday" not in df.columns:
    ...
    # ---- 8b: Constraint-based threshold -- FAR <= 5% (stratified) ----
    print(f"\nConstraint-based: false-alert rate <= 5%")
    ...
    test_cost_cs_far = (C_FP * FP_cs + C_FN * FN_cs) / len(A_test)
    ...
    print(f"Test exp. cost:          {test_cost_cs_far:.4f}")

    # ---- 8c: Constraint-based threshold -- <=2 alerts/week (stratified) ----
    print(f"\nConstraint-based: <= 2 alerts/week (stratified)")
    print(f"  Val set spans {weeks_val:.2f} weeks => max {max_alerts_val} alerts allowed")

    best_tau_2pw_s = 1.0
    feasible_2pw_s = False
    for t in np.linspace(0.999, 0.001, 10000):
        n_alerts_s = (pt_val_strat >= t).sum()
        if n_alerts_s <= max_alerts_val:
            best_tau_2pw_s = t
            feasible_2pw_s = n_alerts_s > 0
            break

    # Find nearest feasible point (fewest alerts > 0)
    nearest_tau_2pw_s = None
    nearest_alerts_2pw_s = None
    for t in np.linspace(0.999, 0.001, 10000):
        n_a = (pt_val_strat >= t).sum()
        if n_a > 0:
            if nearest_alerts_2pw_s is None or n_a < nearest_alerts_2pw_s:
                nearest_tau_2pw_s = t
                nearest_alerts_2pw_s = n_a

    # If strict constraint is infeasible, use nearest feasible operating point
    if not feasible_2pw_s and nearest_tau_2pw_s is not None:
        use_tau = nearest_tau_2pw_s
        print(f"  Strict constraint infeasible (min nonzero alerts = "
              f"{nearest_alerts_2pw_s}, {nearest_alerts_2pw_s/weeks_val:.1f}/wk).")
        print(f"  Using nearest feasible tau = {use_tau:.4f} instead.")
    else:
        use_tau = best_tau_2pw_s

    alerts_2pw_s = (pt_test_strat >= use_tau).astype(int)
    TP_2s = ((alerts_2pw_s == 1) & (A_test == 1)).sum()
    FP_2s = ((alerts_2pw_s == 1) & (A_test == 0)).sum()
    FN_2s = ((alerts_2pw_s == 0) & (A_test == 1)).sum()
    TN_2s = ((alerts_2pw_s == 0) & (A_test == 0)).sum()
    alerts_pw_test_s = alerts_2pw_s.sum() / weeks_test
    test_cost_2pw_s = (C_FP * FP_2s + C_FN * FN_2s) / len(A_test)

    print(f"Chosen tau (val):        {use_tau:.4f}")
    print(f"Test confusion: TP={TP_2s}  FP={FP_2s}  FN={FN_2s}  TN={TN_2s}")
    print(f"Test alerts/week:        {alerts_pw_test_s:.2f}")
    print(f"Test precision:          {TP_2s / (TP_2s + FP_2s + 1e-9):.4f}")
    print(f"Test recall:             {TP_2s / (TP_2s + FN_2s + 1e-9):.4f}")
    print(f"Test exp. cost:          {test_cost_2pw_s:.4f}")

    # ---- Comparison summary ----
    print("\n" + "-" * 80)
    print("COMPARISON: Homogeneous vs. Stratified Model")
    print("-" * 80)
    print(f"{'Metric':<40} {'Homogeneous':>18} {'Stratified':>18}")
    print("-" * 80)
    print(f"{'Brier score (raw, test)':<40} {brier_raw_test:>18.6f} {brier_strat_raw:>18.6f}")
    print(f"{'Brier score (calibrated, test)':<40} "
          f"{min(brier_platt_test, brier_iso_test):>18.6f} "
          f"{min(brier_platt_s, brier_iso_s):>18.6f}")
    print(f"{'Cost-based tau':<40} {tau_cost:>18.4f} {tau_cost_s:>18.4f}")
    print(f"{'Cost-based precision (test)':<40} "
          f"{TP / (TP + FP + 1e-9):>18.4f} "
          f"{TP_s / (TP_s + FP_s + 1e-9):>18.4f}")
    print(f"{'Cost-based recall (test)':<40} "
          f"{TP / (TP + FN + 1e-9):>18.4f} "
          f"{TP_s / (TP_s + FN_s + 1e-9):>18.4f}")
    print(f"{'Cost-based exp. cost (test)':<40} {test_cost:>18.4f} {test_cost_s:>18.4f}")
    print(f"{'Constraint (FAR<=5%) tau':<40} {best_tau_c:>18.4f} {best_tau_c_s:>18.4f}")
    print(f"{'Constraint (FAR<=5%) recall':<40} "
          f"{TP_c / (TP_c + FN_c + 1e-9):>18.4f} "
          f"{TP_cs / (TP_cs + FN_cs + 1e-9):>18.4f}")
    print(f"{'Constraint (FAR<=5%) cost':<40} {test_cost_c:>18.4f} {test_cost_cs_far:>18.4f}")
    print(f"{'Constraint (<=2/wk) tau':<40} {best_tau_2pw:>18.4f} {use_tau:>18.4f}")
    print(f"{'Constraint (<=2/wk) precision':<40} "
          f"{TP_2 / (TP_2 + FP_2 + 1e-9):>18.4f} "
          f"{TP_2s / (TP_2s + FP_2s + 1e-9):>18.4f}")
    print(f"{'Constraint (<=2/wk) recall':<40} "
          f"{TP_2 / (TP_2 + FN_2 + 1e-9):>18.4f} "
          f"{TP_2s / (TP_2s + FN_2s + 1e-9):>18.4f}")
    print(f"{'Constraint (<=2/wk) alerts/week':<40} "
          f"{alerts_per_week_test:>18.2f} "
          f"{alerts_pw_test_s:>18.2f}")
    print(f"{'Constraint (<=2/wk) cost':<40} {test_cost_2pw:>18.4f} {test_cost_2pw_s:>18.4f}")
