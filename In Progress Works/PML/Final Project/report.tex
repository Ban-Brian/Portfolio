...
\textbf{Mean-field limitations.} The factorised Gaussian posterior ignores posterior correlations between weights, which is known to underestimate marginal posterior variances and therefore compress predictive uncertainty intervals (Blundell et al., 2015). The VBNN's uncertainty estimates should thus be interpreted as \emph{lower bounds} on true model uncertainty. A comparison against MC dropout (Gal and Ghahramani, 2016) or a deep ensemble would provide a useful calibration anchor; I report held-out 90\% prediction interval coverage in \S5.3 as a partial check.
...
The five decision bands, with band sizes, are:

\begin{center}\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lcccc@{}}
\toprule
Band & $p(\text{open})$ range & $n$ & Share & Obs.\ closure rate \\
\midrule
Strong avoid & $[0.00, 0.30)$ & 412 & 6.5\% & 78\% \\
Caution      & $[0.30, 0.50)$ & 891 & 14.1\% & 54\% \\
Neutral      & $[0.50, 0.70)$ & 1,204 & 19.0\% & 31\% \\
Favourable   & $[0.70, 0.85)$ & 980 & 15.5\% & 16\% \\
Strong       & $[0.85, 1.00]$ & 2,841 & 44.9\% & 9\% \\
\bottomrule
\end{tabular}
\end{center}

Restaurants whose epistemic survival variance exceeded the 90th percentile had an error rate 2.1$\times$ the global mean, making them natural candidates for a manual-review queue.
...
Second, \textbf{the hierarchical model and the VBNN answer different questions, and neither dominates the other}. The hierarchical model earns its $210\times$ compute cost through interpretable posteriors on every coefficient and group intercept. The VBNN earns its $80\times$ cost through calibrated predictive distributions and uncertainty decomposition. XGBoost, at $3\times$ cost, nearly matches the VBNN on point accuracy.
...
\begin{enumerate}
  \item \textbf{Survival modelling.} The \texttt{is\_open} field is right-censored, and the age covariate is only a partial correction. A Weibull AFT or Cox proportional-hazards model with explicit censoring would be more appropriate. The current binary treatment likely attenuates the strength of time-varying covariates and inflates the apparent predictive power of age.
  ...
  \item \textbf{Mean-field VI.} The factorised Gaussian approximate posterior underestimates marginal variances, yielding prediction intervals that are systematically too narrow (87.3\% vs.\ 90\% nominal coverage). A deep ensemble or full-rank variational family would provide a stronger uncertainty baseline.
  ...
  \item \textbf{External validity.} Only five cities were analysed, and no temporal or geographic hold-out was conducted. City-level inferences should be read as illustrative, not as population-level claims. A stronger validation would train on 4 cities and test on the 5th, or train on data before a cutoff date and test after.
...
A natural extension would be a hierarchical BNN---a neural network whose top-layer weights are grouped by city and cuisine with partial pooling---which would unify the inferential and predictive strengths of the two models. Domain-fine-tuned embeddings, a causal rather than purely predictive framing with explicit DAGs, proper longitudinal survival modelling, and cross-city temporal hold-out validation would address the deepest limitations of the current approach.
